...
const VideoCallModal = () => {
  ...
  const { callState, endCall, startCall, answerCall , contextSwitchCamera ,initialState } = useCall();
  ...
  socket.on("ringing",handleRinging)
      ...
      const handleEndCall = ({from}) => {
        if(from?.toString() === callState?.callerId?.toString() && callState.iceConnection !== "disconnected")
        {
  ...
  useEffect(() => {
    if (localVideoRef.current && !location.state?.incoming) {
      navigator.mediaDevices
      .getUserMedia({ video: true, audio: true })
      .then((stream) => {
          localVideoRef.current.srcObject = stream;
          startCall(participant?._id, "video", stream,user.id , setAlert ); 
        })
        .catch((err) => {
          let message = "Error accessing media devices.";
          ...
          if (err.name === "NotAllowedError") {
            ...
            message =
              "Could not access your camera or microphone. It may be in use by another app.";
          } else if (err.name === "OverconstrainedError") {
            message = "No media devices match the requested constraints.";
          }
  ...
  const switchCamera = () => {
    ...
    navigator.mediaDevices
      ...
      .then((newStream) => {
        ...
        contextSwitchCamera(newStream);
      })
      .catch((err) => {
        setAlert({ msg: "Error while switching camera:", type: "error" });
          ...
          <div className="incoming-call">
            ...
            <div className="call-actions">
              <button onClick={handleAcceptCall}>
                <img src="/svg/callaccept.svg" alt="accept call" />
              </button>
              <button style={{ backgroundColor: "red" }} onClick={()=>{
                socket.emit("call-rejected",{to:callState.callerId});
                endCall()}}>
                <img src="/svg/calldecline.svg" alt="decline call" />
              </button>
            </div>
          </div>
        )}
  ...
  />
            ...
            <div className="call-screens">
              <div className="screen">
                ...
                {!callState.callAccepted &&<span>{callResponse === false?"The user is offline , not reachable at this time.":callResponse === true?"Ringing":null}</span>}
                {callState.callAccepted && callState.iceConnection === "" &&<span>connceting</span>}
              </div>

               <div className="screen" style={{ display: callState.callAccepted && callState.iceConnection === "connected" ? 'block' : 'none' }} >
            ...
            <div className="call-actions">
              {isMobile && (
                <button onClick={switchCamera}>
                  <img src="/svg/switchcamera.svg" alt="switch camera" />
                </button>
              )}
              <button style={micOff?{opacity:"0.5"}:{opacity:"1"}} onClick={toggleMicrophone}>
                <img src="/svg/microphoneoff.svg" alt="microphone off" />
              </button>
              <button style={camOff?{opacity:"0.5"}:{opacity:"1"}} onClick={toggleCamera}>
                <img src="/svg/videocamoff.svg" alt="videocam off" />
              </button>
              <button style={{ backgroundColor: "red" }} onClick={()=>{
              endCall();
              }}>
                <img src="/svg/endcall.svg" alt="end call" />
              </button>
            </div>
